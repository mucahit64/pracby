<template>
  <div class="min-h-screen bg-gray-900 text-gray-100 flex">
    ...
    <div v-if="isMobile" class="fixed top-0 left-0 right-0 z-30 bg-gray-800 border-b border-gray-700 px-4 py-3 flex items-center justify-between">
      ...
      <span class="font-bold text-lg text-purple-400">Pracby Admin</span>
      <button @click="logout" class="text-gray-400 hover:text-red-400 text-sm">Çıkış</button>
    </div>
    ...
    <aside
      :class="[
        'fixed lg:static z-50 top-0 left-0 h-full w-64 bg-gray-800 border-r border-gray-700 flex flex-col transition-transform duration-200',
        isMobile && !sidebarOpen ? '-translate-x-full' : 'translate-x-0'
      ]"
    >
      <div class="px-6 py-5 border-b border-gray-700">
        <h1 class="text-xl font-bold text-purple-400">Pracby Admin</h1>
      </div>
...
onMounted(async () => {
  ...
  if (token) {
    try {
      const user = await $fetch<{ email?: string }>('/api/users/me', {
