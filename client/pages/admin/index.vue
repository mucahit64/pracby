    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="card in cards" :key="card.label" class="bg-gray-800 rounded-xl p-5 border border-gray-700">
        <div class="flex items-center justify-between mb-2">
          ...
          <component :is="card.icon" class="w-5 h-5 text-gray-500" />
        </div>
...
// Inline SVG icons as render functions so they render as real <svg> elements
// instead of being escaped as plain text strings via {{ }} interpolation.
const icon = (d: string): FunctionalComponent => () =>
...
const cards = computed(() => [
  ...
  },
  {
    ...
    icon: icon('M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z'),
  },
])
