...
export async function seed(knex: Knex): Promise<void> {
  ...
  if (existing) {
    console.log("005: KPSS Lisans Genel Kültür (Tarih) içeriği zaten mevcut, atlanıyor.");
    return;
  }
  ...
  if (gkModules.length === 0) {
    throw new Error("005: KPSS Lisans Genel Kültür modülü bulunamadı — 003_exam_structure seed çalıştırıldı mı?");
  }
