...
// Lisans içeriklerini geçici olarak Önlisans için import ediyoruz
...
// ============================================================
// SEED 012 — KPSS Önlisans Tüm Dersler (GY & GK)
// ============================================================
...
export async function seed(knex: Knex): Promise<void> {
  ...
  for (const data of coursesData) {
    // 1. İlgili modülü (Genel Yetenek veya Genel Kültür) KPSS Önlisans için bul
    const moduleRecord = await knex("modules")
      ...
      .where("exam_types.slug", "onlisans")
    ...
    if (!moduleRecord) {
      console.warn(`012: KPSS Önlisans ${data.moduleName} modülü bulunamadı, ${data.courseName} dersi atlanıyor.`);
      continue;
    }
    ...
    if (!course) {
      console.warn(`012: KPSS Önlisans ${data.moduleName} modülünde ${data.courseName} dersi bulunamadı.`);
      continue;
    }
    ...
    if (existing) {
      console.log(`012: KPSS Önlisans ${data.moduleName} - ${data.courseName} içeriği zaten mevcut, atlanıyor.`);
      continue;
    }
    ...
    console.log(`012: KPSS Önlisans ${data.moduleName} - ${data.courseName} içeriği başarıyla eklendi.`);
  }
}